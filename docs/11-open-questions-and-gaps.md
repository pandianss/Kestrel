...
*2026-07-24 — a third factor: value.* `kestrel/strategies/value.py` implements value (earnings/book yield) to the same contract, backed by point-in-time fundamentals (`kestrel/data/fundamentals.py`) whose `asof()` returns only what was **public** by the scoring date — the value look-ahead guard. Unlike momentum/low-vol it cannot run on Yahoo prices alone: **Kite provides no fundamentals**, so a trustworthy value backtest waits on a real point-in-time fundamentals feed — an **owner data-source decision** (vendor, screener.in, or filings; cost/licensing/coverage). The factor math and its point-in-time discipline are built and tested against a dev source; only the data is deferred, mirroring how the momentum verdict waits on the point-in-time *universe* (G-43).

*2026-07-23 — a second factor, for comparison.* Low-volatility (`kestrel/strategies/low_volatility.py`) is now implemented to the same contract as momentum, and `scripts/run_factor_comparison.py` runs them head to head. On the same survivor pool: the two factors' net returns correlate **+0.71** (related, not identical bets), and low-vol *underperforms* the equal-weight-hold control (**IR −0.50**) — on a universe of only mega-cap survivors, selecting the calmest names forfeits the high-momentum survivors' returns. **Neither factor's ranking is trustworthy on this data**; the comparison harness exists so that when the point-in-time universe arrives, "which factor is real on our market" is a question the code can already answer, not one still to be built. Choosing *between* factors is still an owner decision (this note does not make it) — the value here is the apparatus, not the survivor-biased numbers.
